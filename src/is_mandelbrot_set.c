...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/28 06:40:04 by ikawamuk          #+#    #+#             */
/*   Updated: 2025/06/28 12:31:56 by ikawamuk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "is_set.h"

static t_complex	mandelbrot_relation(t_complex z, t_complex c);
...
int	is_mandelbrot_set(t_complex c, t_ctx *ctx)
{
	...
	return (is_divergent_set(z, c, mandelbrot_relation, ctx));
}
...
static void	mandelbrot_max_iter(t_ctx *ctx)
{
	...
	scale = 4.0 / ctx->width;
	ctx->max_iter = (int)(30 + log2(scale) * 30);
	if (ctx->max_iter < 100)
		ctx->max_iter = 100;
	return ;
}
